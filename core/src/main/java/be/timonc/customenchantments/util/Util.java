package be.timonc.customenchantments.util;
...
public final class Util {
    ...
    public static String replaceParameters(Player player, String value, Map<String, Supplier<String>> parameters) {
        ...
        do {
            ...
            Matcher matcher = PARAM_PATTERN.matcher(current);
            ...
            while (matcher.find()) {
                ...
                Supplier<String> localSupplier = parameters.get(paramName);
                if (localSupplier != null)
                    replacement = localSupplier.get();
                else if (Main.isPAPISupport()) {
                    String papiResult = PlaceholderAPI.setPlaceholders(player, fullMatch);
                    if (!papiResult.equals(fullMatch))
                        replacement = papiResult;
                }
                ...
                if (replacement != null) {
                    ...
                    matcher.appendReplacement(resultBuffer, Matcher.quoteReplacement(fullMatch));
            }
            matcher.appendTail(resultBuffer);
            current = resultBuffer.toString();
        } while (changed);

        // Check for unresolved parameters
        Matcher finalMatcher = placeholderPattern.matcher(current);
        if (finalMatcher.find())
            Util.warn("Unresolved placeholders found in text: " + current);
